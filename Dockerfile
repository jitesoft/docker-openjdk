...
ARG JAVA_FILE="openjdk-14-ea+8_linux-x64-musl_bin.tar.gz"
ENV JAVA_HOME="/jdk" \
    LANG="C.UTF-8" \
    PATH="$PATH:/jdk/bin"

RUN wget -O /openjdk.tar.gz https://download.java.net/java/early_access/alpine/8/binaries/${JAVA_FILE} \
 && wget -qO- https://download.java.net/java/early_access/alpine/8/binaries/${JAVA_FILE}.sha256 | xargs printf "%s */openjdk.tar.gz" | sha256sum -c - \
 && mkdir /jdk \
